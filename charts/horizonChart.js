(function(){
    ...
    var chart = raw.chart()
        ...
        .description("Horizon charts combine position and color to reduce vertical space. Start with a standard area chart, then mirror negative values (in blue) or offset them vertically. Click the + button above to increase the number of bands, turning the area into a horizon.<br/>Based on <a href='http://bl.ocks.org/mbostock/9490313'>http://bl.ocks.org/mbostock/1483226</a>")
	...
	var mode = chart.list()
        .title("Render mode")
        .values(['offset', 'mirror'])
        .defaultValue('offset')
    
    var interpolate = chart.list()
        .title("Interpolation")
        .values(['linear', 'basis', 'monotone', 'step-before'])
        .defaultValue('linear')
